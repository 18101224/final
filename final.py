...
ax.view_init(10,-10)
ax.scatter3D(np.linspace(403,len(pcr)-1,len(data['positive'])),data['vaccine'],data['positive'])
...
for item in ('AZ','Fizer','Modern'):
    for i in range(91,len(data[item])):
        data[item][i]=data[item][i]-data[item][90] #항체감소 보정
fomular3='deathRate~AZ+Fizer+Modern'
result3=smf.ols(fomular3,data).fit()
print("보정 후 결과\n",result3.summary())
def deathForVaccine2(A,F,M):
    return -9.101e+04*A-1.638e+05*F+1.007e+06*M+6.05e+04
